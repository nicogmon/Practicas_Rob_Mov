...
def compute_particle_weights(gui_obj,particles):
    ...
    for particle in particles:
        ...
        if map_particle[0] < 0 or map_particle[1] < 0 or map_particle[0] >= MAP.MAP_WIDTH or map_particle[1] >= MAP.MAP_HEIGHT:
            ...
            continue
        #if mapa[map_particle[0]][map_particle[1]] == OBSTACLE_VALUE:
    ...
    for i, laser in enumerate(lasers):
        lasers[i] = np.concatenate((laser, laser_ref))
    """for laser in lasers:
        #convertir a mapa las coordenadas 
    ...
    pool2 = mp.Pool(processes=num_processes)
    weights = pool.map(compare_laser,lasers)
